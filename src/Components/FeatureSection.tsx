...
const FeatureSection = () => {
  ...
  // Transform values for text opacity based on scroll position
  ...
  useEffect(() => {
    // Set active card based on scroll progress
    const unsubscribe = scrollYProgress.onChange(value => {
      // Calculate which card should be active based on scroll progress
      // Dividing the scroll range equally among the cards (minus some buffer space)
  ...
  return (
    <div 
      ...
      style={{ height: "400vh" }} // Fixed height for consistent scrolling experience
    >
      <div className="sticky top-0 w-full h-screen overflow-hidden flex flex-col lg:flex-row">
        ...
        >
          <div className="max-w-xl">
            ...
            <motion.p 
              className="text-lg text-[#E9D9FE] mb-8"
            >
        ...
        <div className="w-full lg:w-1/2 relative flex flex-col items-center justify-center px-6 py-12">
          <div className="w-full max-w-md relative">
            {featureCards.map((card, index) => (
              <motion.div
                key={card.id}
                className="w-full absolute top-0 left-0 bg-[#F5F7FF] rounded-xl shadow-xl p-8 mb-8 transform origin-top"
                animate={{
                  opacity: activeCardIndex === index ? 1 : 0.3,
                  scale: activeCardIndex === index ? 1 : 0.9,
                  y: (index - activeCardIndex) * 30,
          ...
          <div className="absolute bottom-8 flex space-x-2">
            {featureCards.map((_, index) => (
              <motion.div 
                key={index}
                className="w-3 h-3 rounded-full bg-[#F5F7FF]/20 cursor-pointer"
                animate={{
